...
page_url = "https://www.hinditracks.in/"
song_post_url =""
...
song_list = page_soup.find_all("a", rel="bookmark")
...
for i in song_list:
    ...
    try:
        ...
        song_data["release_date"] = song_page_soup.find("time", "entry-date published updated").attrs["datetime"]

        song_data["dic"] = song_page_soup.find("div", "post-inner-wrap").find("p").text
        ...
        img_link = song_page_soup.find("table", "youtube-info").find("img").attrs["data-rt-lazy"]

        image_data = request_session.get(img_link).content

        song_img = {"song-image": (song_data["name"]+"-song.jpg", image_data)}

        temp = ""
        list_p = song_page_soup.find("div", id="englishlyrics").find_all("p")
        for k in list_p:
            temp = temp+str(k)
